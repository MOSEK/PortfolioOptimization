        ...
        with Model("Case study") as M:
            ...
            for d in deltas:
                ...
                row = pd.Series([d, M.primalObjValue(), portfolio_return, portfolio_risk, 
                                 sum(t_resid), sum(x.level()), sum(xf.level()), t.level() @ a] + list(x.level()), 
                                 index=columns)
...
def _(N, np, volatility, volume):
    ...
    a2 = (_c * volatility / rel_volume ** (beta - 1)).to_numpy()
    return a1, a2, beta, rf
...
@app.cell
def _(EfficientFrontier, G, N, a1, a2, beta, m, np, plt, rf):
