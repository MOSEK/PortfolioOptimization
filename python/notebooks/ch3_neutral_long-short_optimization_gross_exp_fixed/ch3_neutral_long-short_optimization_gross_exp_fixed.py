    ...
    def EfficientFrontier(N, m, G, deltas):
        ...
        with Model("Case study") as M:
            ...
            for d in deltas:
                ...
                row = pd.Series([d, M.primalObjValue(), portfolio_return, portfolio_risk, gross_exp, time_spent] + list(x.level()), 
                                index=columns)
...
def _(EfficientFrontier, G, N, m, np):
    ...
    deltas = np.logspace(start=-1, stop=1.5, num=20)
...
def _(LinearSegmentedColormap, df_result, plt):
    ...
    _ax = df_result.set_index('risk').iloc[:, 5:].plot.bar(colormap=my_cmap, xlabel='portfolio risk (std. dev.)', ylabel='x', stacked=True, width=1.0)
