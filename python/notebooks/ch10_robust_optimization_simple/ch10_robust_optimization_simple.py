...
@app.cell
def _(EfficientFrontier, G, GQ, N, gamma, gamma_z, mu0, np):
...
def _(LinearSegmentedColormap, df_result_orig, df_result_robust, plt):
    ...
    ax1 = df_result_robust.set_index('risk').iloc[:, 3:].clip(0, None).plot.area(colormap=my_cmap, xlabel='portfolio risk (std. dev.)', ylabel="x")
    ax1.grid(which='both', axis='x', linestyle=':', color='k', linewidth=1)
    ax2 = df_result_orig.set_index('risk').iloc[:, 3:].clip(0, None).plot.area(colormap=my_cmap, xlabel='portfolio risk (std. dev.)', ylabel="x") 
