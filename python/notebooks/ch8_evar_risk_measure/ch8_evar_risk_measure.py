...
def _(
    ...
    def persplogsumexp(M, x, s, p, t):
        n = int(x.getSize())
        u = M.variable(n)
        M.constraint(Expr.hstack(u, Var.repeat(s, n), x - Var.repeat(t, n)), Domain.inPExpCone())
        M.constraint(u.T @ p <= s)
    ...
    def EfficientFrontier(N, T, m, R, p, alpha, deltas):
        ...
        with Model("Case study") as M:
            ...
            persplogsumexp(M, -R * x, t, p, z)

            # Objective
            delta = M.parameter()
            evar_term = z - t * np.log(1.0 - alpha)
            M.objective('obj', ObjectiveSense.Maximize, x.T @ m - delta * evar_term)
            ...
            for d in deltas:
                ...
                row = pd.Series([d, M.primalObjValue(), portfolio_return, portfolio_risk, cvar] + list(x.level()), 
                                index=columns)
...
@app.cell
def _(EfficientFrontier, N, T, alpha, m, np, p, scenarios_lin):
...
@app.cell
def _(df_result):
    _ax = df_result.plot(x='risk', y='return', style='-o', xlabel='portfolio risk (EVaR)', ylabel='portfolio return', grid=True)
    _ax.legend(['return'])
    return
...
def _(LinearSegmentedColormap, df_result, plt):
    ...
    _ax = df_result.set_index('risk').iloc[:, 4:].plot.area(colormap=my_cmap, xlabel='portfolio risk (EVaR)', ylabel='x')
