...
def _(
    ...
    def EfficientFrontier(N, T, m, R, p, alpha, deltas):
        ...
        with Model("Case study") as M:
            ...
            M.objective('obj', ObjectiveSense.Maximize, x.T @ m - delta * cvar_term)
...
@app.cell
def _(df_result):
    _ax = df_result.plot(x='risk', y='return', style='-o', xlabel='portfolio risk (CVaR)', ylabel='portfolio return', grid=True)
    _ax.legend(['return'])
    return
...
def _(LinearSegmentedColormap, df_result, plt):
    ...
    _ax = df_result.set_index('risk').iloc[:, 3:].plot.area(colormap=my_cmap, xlabel='portfolio risk (CVaR)', ylabel='x')
